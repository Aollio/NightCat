export class Emoji {
    ...
    public static emojiBaseUrl = `http://yx-web.nosdn.127.net/webdoc/h5/emoji`

    public static pinupList = {
        'ajmd': {},
        'xxy': {},
        'lt': {}
    }
    ...
    static initalize() {
        ...
        for (let emoji in this.emojiList) {
            ...
            for (let key in emojiItem) {
                ...
                item.img = `${this.emojiBaseUrl}/${emoji}/${item.file}`
            }
        }
        ...
        for (let i = 1; i <= 48; i++) {
            ...
            this.pinupList['ajmd'][key] = {file: key + '.png'}
        }
        for (let i = 1; i <= 40; i++) {
            let key = 'xxy0' + (i >= 10 ? i : '0' + i)
            this.pinupList['xxy'][key] = {file: key + '.png'}
        }
        for (let i = 1; i <= 20; i++) {
            let key = 'lt0' + (i >= 10 ? i : '0' + i)
            this.pinupList['lt'][key] = {file: key + '.png'}
        }

        for (let emoji in this.pinupList) {
            let emojiItem = this.pinupList[emoji]
            for (let key in emojiItem) {
                let item = emojiItem[key]
                item.img = `${this.emojiBaseUrl}/${emoji}/${item.file}`
            }
